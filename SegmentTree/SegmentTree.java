...
public class SegmentTree {
    ...
    private void buildTree(int[] arr, int tIdx, int left, int right) {
        ...
        tree[tIdx] =  tree[leftChild] + tree[rightChild];
    }

    public int getSum(int stLeft, int stRight, int qLeft, int qRight, int currIdx) {
        // complete overlap
        if(qLeft <= stLeft && qRight >= stRight) return tree[currIdx];

        // no overlap
        if(qLeft > stRight || qRight < stLeft) return 0;

        int mid = (stLeft + stRight) / 2;

        // partial overlap
        return getSum(stLeft, mid, qLeft, qRight, 2*currIdx + 1) +
            getSum(mid + 1, stRight, qLeft, qRight, 2*currIdx + 2);
    }
    ...
    public static void main(String[] args) {
        ...
        int rangeSum = st.getSum(0, 5, 2, 4, 0);
        System.out.println("The sum in give range: " + rangeSum);

        st.updateValue(arr, 3, 14);
        System.out.println("The total sum of array: " + st.tree[0]);
